...
export class Query {
    ...
    findById(): object {
        return {
            ...
            type: `SELECT_ONE`,
            syntax: (where: any) => {
                const allowedKeys = ['id_city', 'name', 'state_id', 'status'];
    ...
    findByName(id?: string): object {
        return {
            ...
            syntax: (where: any) => {
                ...
                const condition = id ? `AND id_city != '${id}'` : '';
                const sql = `SELECT ${allowedKeys.join(', ')} FROM city_mas WHERE status = 1 AND name = '${name}' ${condition};`;
                return sql;
            },
        };
    }
    ...
    insert(): object {
        return {
            ...
            type: `INSERT`,
            syntax: (where: any) => {
                const allowedKeys = ['name', 'state_id', 'status'];
                const conds = _.pick(where, allowedKeys);
                const keys = _.keys(conds);
                const values = _.values(conds);
                const sql = `INSERT INTO city_mas (${keys.join(', ')}) VALUES ('${values.join("','")}') RETURNING id_city as insertid, name;`;
                return sql;
            },
        };
    }
